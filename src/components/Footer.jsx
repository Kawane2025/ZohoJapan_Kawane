...
export default function Footer(){
  ...
  const mapSrc = `https://www.google.com/maps?q=${
    encodeURIComponent(COMPANY.mapsQuery || COMPANY.addressLines.join(' '))
  }&output=embed`;
    ...
    <footer className="mt-10 bg-zoho-navy text-white">
      <div className="max-w-6xl mx-auto px-4 py-6 grid md:grid-cols-2 gap-6 items-start">
        ...
        <div>
          <div className="flex items-start gap-3 mb-4">
            ...
            <div>
              <div className="text-xl font-semibold">ゾーホージャパン川根本</div>
              <div className="text-sm opacity-80">地域と学びをつなぐコミュニティプログラム</div>
            </div>
          </div>
          <div className="space-y-1 text-base opacity-95">
            <div className="font-semibold">{COMPANY.name}</div>
            {COMPANY.addressLines.map((l,i)=>(<div key={i}>{l}</div>))}
            <div>Phone: <a className="underline" href={`tel:${COMPANY.phone}`}>{COMPANY.phone}</a></div>
          </div>
        </div>
        ...
        <div className="max-w-6xl mx-auto px-4 py-6 flex flex-col md:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            <a href="https://www.zoho.co.jp/kawanehoncho/" target="_blank" rel="noopener noreferrer"
               className="inline-flex items-center px-4 py-2 rounded-lg bg-white/10 hover:bg-white/15">
              川根の活動を見る２
            </a>
            <a href="https://www.zoho.com/jp/" target="_blank" rel="noopener noreferrer"
               className="inline-flex items-center px-4 py-2 rounded-lg bg-white/10 hover:bg-white/15">
              Zohoについてもっと知る
            </a>
          </div>
      ...
      <div className="">
        <div className="max-w-6xl mx-auto px-4 py-3 text-xs opacity-90 flex items-center justify-between">
          ...
          <a href="https://www.zoho.co.jp/kawanehoncho/" target="_blank" rel="noopener noreferrer" className="underline">活動を見る</a>
        </div>
      </div>
    </footer>
  )
