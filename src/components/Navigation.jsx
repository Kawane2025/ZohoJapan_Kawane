...
export default function Navigation() {
  ...
  const NavLink = ({ href, children }) => (
    <a href={href} className="px-3 py-2 hover:text-zoho-blue text-gray-800">
      {children}
    </a>
  );
  return (
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex items-center gap-1 px-3 py-2 text-sm border rounded-lg"
            >
          ...
          {open && (
            ...
            >
              <div className="flex flex-col">
                <a href="#events" className="px-3 py-2">
                  {language === "ja" ? "イベント" : "Events"}
                </a>
                <a href="#registration" className="px-3 py-2">
                  {language === "ja" ? "登録" : "Registration"}
                </a>
                <a href="#gallery" className="px-3 py-2">
                  {language === "ja" ? "ギャラリー" : "Photo Gallery"}
                </a>
              </div>
            </motion.div>
          )}
